...
<script language="JavaScript" type="text/javascript" >
  $(document).ready(function () {   
     $('#criterio').change(function(){
        ...
        if($('#criterio').val()=="OTROS"){
            $('#criterio_otro').show();
            $('#criterio_otro').removeAttr("disabled");
        }
        else
        {
            $('#criterio_otro').hide();
            $('#criterio_otro').val("");
            $('#criterio_otro').attr("disabled","disabled");
        }
    });  
  });
  ...
  function guarda_riesgo(valor) {
   ...
   if(valor == 6) {
      ...
      if( ! fTrim( $('#valor').val() ) ){
        ...
        if($('#criterio').val()==""){
             Mensaje = Mensaje + "'CRITERIO' es obligatorio\n";
        }
        else
        {
            if($('#criterio').val()=="OTROS" && !fTrim($('#criterio_otro').val())){
                Mensaje = Mensaje + "Si el Criterio seleccionado es OTROS, debe registrar la Descripci\363n del Criterio en la casilla inferior\n";
            }         
        }
...
<html:form action="/VariableRiesgoAdmin" focus="gestion" onsubmit="return false;">
     ...
     <% if(estado.equals("VARIABLE")) { %>   
             ...
             try
              { sql.getConexion();
                ...
                <table width="90%" border="1" cellpadding="3" cellspacing="0" class="marco" align="center" id="c">
                  ...
                  if( ! ( rs == null || !rs.next() ) ) {
                    do {
                        lnkd="deshabilitar(\""+rs.getString(1)+"\",\""+rs.getString(11)+"\");";
                        lnkh="habilitar(\""+rs.getString(1)+"\",\""+rs.getString(11)+"\");";
            ...
            %> 
                    <tr class="<%=var%>">
                        ...
                        <td>
                        <% if(rs.getString( 3).equals("HABILITADO") )
                        {%> 
                        <input type="button" value="Deshabilitar" name="boton" onclick='<%=lnkd%>'></input>
                        <%}else{%>
                        <input type="button" value="Habilitar" name="boton" onclick='<%=lnkh%>'></input>
                        <%}%>
                        </td>
                    </tr>
     ...
     <% if(estado.equals("ADICIONA")) { %>
            ...
            <% if(bean.getTipo_variable().equals("NIT")) { %>
                ...
                <tr>
                    <th>Criterio</th>
                    <td>
                        <html:select property="criterio" styleId="criterio" >
                            <html:option value="">-- Seleccione un Criterio --</html:option>                        
                            <html:option value="DEMASIA">DEMASIA</html:option>                               
                            <html:option value="CONTRABANDO">CONTRABANDO</html:option>                                
                            <html:option value="ROPA USADA">ROPA USADA</html:option>                                
                            <html:option value="MERCANCIA PROHIBIDA">MERCANCIA PROHIBIDA</html:option>                                
                            <html:option value="FRAUDE BANCARIO">FRAUDE BANCARIO</html:option>                                
                ...
                <tr>
                    <th>&nbsp;</th>
                    <td>
                        <html:text property="criterio_otro" style="display:none" disabled="true" styleId="criterio_otro" maxlength="50" size="20"/>
                    </td>
                </tr>
                <tr>
