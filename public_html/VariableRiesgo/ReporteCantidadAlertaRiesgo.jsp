...
<input type="hidden" name="boton" id="boton"/>
  <table width="50%" border="0" cellpadding="4" cellspacing="0" class="marco" id="a">
    <caption><label style="font-size:15px">Reportes Estadisticos - N&uacute;mero de Alertas Generadas Riesgo Resultados y Estado</label></caption>
      ...
      <%if(bean == null || bean.getFec_hasta() == null)
  {
     ...
     <tr><th>Tipo Variable:</th>
            <td  colspan="3">
                <html:select property="tipo_var">
                    <html:option value="%">&lt;-- Todas las Variables --&gt;</html:option>
                    <html:option value="NIT">NIT EMPRESA DE TRANSPORTE</html:option>
                    <html:option value="PLACA">PLACA</html:option>
                    <html:option value="REMITENTE">REMITENTE (PROVEEDOR)</html:option>
 ...
 try
  { sql.getConexion();
    ...
    call = sql.cn.prepareCall( "{? = call PKG_VARIABLE_RIESGO.reporte5( ?,?,? ) }" );
    call.registerOutParameter( 1, OracleTypes.CURSOR );
    call.setString( 2, bean.getFec_desde() );  
    call.setString( 3, bean.getFec_hasta() ); 
    call.setString( 4, bean.getTipo_var()); 
    ...
    <table width="90%" border="0" cellpadding="3" cellspacing="0" align="center">
        <tr>
            <th><label style="font-size:15px">N&uacute;mero de Alertas Generadas Riesgo Resultados y Estado</label></th>
        </tr>
    </table>
    <table width="90%" border="1" cellpadding="3" cellspacing="0" class="marco" align="center" id="c">
        <tr class="t14">
            <th align="center" >RIESGOS</th>
      ...
      if( ! ( rs == null || !rs.next() ) ) {
        ...
        total_ale = total_ale + Integer.parseInt(rs.getString( 2 ));
        total_obs = total_obs + Integer.parseInt(rs.getString( 3 ));
        total_sin = total_sin + Integer.parseInt(rs.getString( 4 ));
        total_pen = total_pen + Integer.parseInt(rs.getString( 5 ));
        
        total_tot = total_ale + total_obs + total_sin + total_pen;
